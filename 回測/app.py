...
# --- [全域變數與金鑰] ---
# ⚠️ 建議未來將此 Token 移至 .streamlit/secrets.toml 中以確保安全
...
@st.cache_data(ttl=300) # 新增快取機制，降低 API 負載
...
# --- 【BACKTEST：每日真實準確率回測看板 (視覺化升級版)】 ---
elif st.session_state.mode == "backtest":
    if st.button("⬅️ 返回首頁"): st.session_state.mode = "home"; st.rerun()
    st.title("📊 每日預測紀律與真實準確率回測中心")
    
    with st.spinner('正在對齊歷史數據並繪製準確率視覺圖表...'):
        df_db, total_acc = update_and_calculate_accuracy()
        if df_db.empty:
            st.warning("📭 目前無歷史預測紀錄！請先至「隔日區間預估」儲存您的觀察標的。")
        else:
            # 濾除尚未開獎的資料，只拿來做準確率圖表
            df_closed = df_db[df_db['is_hit'].isin(["Hit (成功)", "Miss (未命中)"])].copy()
            
            # --- [見] 核心數據看板 ---
            hit_count = len(df_closed[df_closed['is_hit'] == "Hit (成功)"])
            miss_count = len(df_closed[df_closed['is_hit'] == "Miss (未命中)"])
            ...
            st.markdown(f"""
                <div style="background: linear-gradient(135deg, #1E1B4B 0%, #431407 100%); padding: 25px; border-radius: 16px; color: white; margin-bottom: 25px;">
                    ...
                    <span style="font-size: 16px; color: #E2E8F0;">總結算筆數：{total_count} 筆 ｜ 🎯 命中：{hit_count} ｜ ❌ 失誤：{miss_count}</span>
                </div>
            """, unsafe_allow_html=True)
            ...
            if total_count > 0:
                ...
                with col_chart1:
                    st.subheader("📈 [識] 每日預測勝率趨勢")
                    # 計算每日勝率
                    daily_stats = df_closed.groupby('prediction_date').apply(
                        lambda x: (len(x[x['is_hit'] == "Hit (成功)"]) / len(x)) * 100
                    ).reset_index(name='Daily_Accuracy')
                    
                    daily_stats['prediction_date'] = pd.to_datetime(daily_stats['prediction_date'])
                    daily_stats = daily_stats.set_index('prediction_date')
                    
                    st.line_chart(daily_stats['Daily_Accuracy'], color="#34D399", height=300)
                    
                with col_chart2:
                    st.subheader("🎯 [謀] 預測結果結構佔比")
                    ...
                    if sum(sizes) > 0:
                        ...
                        st.info("目前數據量不足以繪製圓餅圖。")

            st.divider()
            st.subheader("📋 [斷] 歷史預測明細清單")
